    ...
    def on_train_end(self, logs={}):
        ...
        if self.acc_or_loss == 'acc':
            ...
            plt.title('Training and Validation Accuracy for ' +
                      str(self.epoch_count) + ' epochs of training.')
...
def get_output(model, layer_name, model_input):
    ...
    get_act = K.function([model.layers[0].input,
                          K.learning_phase()],
...
def transform_into_bitmap(images, classes_list):
    ...
    for image in range(0, images.shape[0]):
        ...
        if image % 10 == 0 and image != 0:
            print(str(i-1) + ' of ' + str(images.shape[0]) + ' done. please hold the line.')
...
def augment_images(images, h_flip=True, rotate180=True, shift_random=True):
    ...
    if shift_random:
        ...
        for nb_sample in range(0, images.shape[0], 1):
            ...
            shift_images[nb_sample, :, :, :] = np.roll(images[nb_sample, :, :, :],
    ...
    for nb_image in range(0, predicted_bitmap.shape[0]):
        for row in range(0, predicted_bitmap.shape[1]):
            for column in range(0, predicted_bitmap.shape[2]):
                ...
                class_index = np.argmax(predicted_bitmap[nb_image,row, column, :], axis = 0)
...
def ae_predict(model, im_path, im_gt_path, lower_bound, upper_bound,
               specification, class_or_regr):
    ...
    for index, file_path in enumerate(glob.glob(im_path + '\\*.png')):
        ...
        for i in range(0, 3):
            ...
            if i == 2:
                ...
                plt.imsave('logs/autoencoder_logs/'+specification+'/prediction'+str(index)+'.png'
                           , prediction)
